...
import type { DocsClarifyQuestion } from "./types";
import { startJob, resumeJob as resumeJobBase, type JobHandlers, getJob } from "@/lib/jobs/client";
...
export type DocsStreamHandlers = {
  ...
  onHtmlDone?: (fullHtml: string) => void;
  onClarify?: (c: { reason: string; questions: DocsClarifyQuestion[] }) => void;
...
function toJobHandlers(h: DocsStreamHandlers): JobHandlers {
  return {
    ...
    onClarify: (c) => h.onClarify?.({ reason: c?.reason ?? "", questions: (c?.questions ?? []) as DocsClarifyQuestion[] }),
    onDone: (row) => {
      const html = (row.output?.html as string) || row.stream_text || "";
      if (html) h.onHtmlDone?.(html);
    },
...
export async function streamDoc(
  ...
  await new Promise<void>((resolve, reject) => {
    ...
    const wrapped: DocsStreamHandlers = {
      ...handlers,
      onHtmlDone: (full) => safely(handlers.onHtmlDone, full),
