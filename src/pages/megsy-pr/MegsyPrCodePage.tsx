...
export default function MegsyPrCodePage() {
  ...
  const handleRun = async () => {
    ...
    try {
      if (isRunning || isStarting) {
        ...
        toast.info("جاري تشغيل الـ sandbox… أول مرة بتاخد 1-2 دقيقة لـ npm install");
  ...
  const runPythonBuildCheck = async () => {
    ...
    try {
      if (!isRunning) {
        toast.info("تشغيل E2B sandbox أولاً…");
      ...
      toast.success("تم اختبار Python داخل E2B بنجاح");
    } catch (e) {
          ...
          <div className="flex-1 overflow-auto">
            ...
            {overlayTab === "code" && active.id === "__preview__" && (
              <div className="grid place-items-center h-full text-sm text-muted-foreground">
                اختر ملفاً من القائمة لعرض محتواه هنا.
              </div>
            )}
            {overlayTab === "preview" && (
              sandbox?.dev_url ? (
                ...
                <div className="grid place-items-center h-full text-sm text-muted-foreground p-6 text-center">
                  لا يوجد sandbox شغّال. اضغط Run من أعلى الصفحة لبدء الـ preview.
                </div>
              )
            )}
            {overlayTab === "terminal" && (
              <pre dir="ltr" className="text-[12px] leading-[1.55] font-mono p-4 whitespace-pre-wrap text-foreground/90">
                {logs || (sandbox?.sandbox_id ? "(no logs yet)" : "لا يوجد sandbox شغّال.")}
              </pre>
            )}
          </div>
        </div>
