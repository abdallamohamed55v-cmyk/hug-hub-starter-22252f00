...
export default function MegsyPrWorkspacePage() {
  ...
  useEffect(() => {
    ...
    (async () => {
      ...
      unsub = resumeJob(jobId, {
        ...
        onStale: async (row) => {
          const msg = "عامل البرمجة توقف قبل إنهاء المهمة. تم حفظ المحادثة والملفات، اضغط إرسال لاستكمال نفس المشروع.";
          try { localStorage.removeItem(`buildAgent:activeJob:${projectId}`); } catch { /* ignore */ }
    ...
    try {
      ...
      const handleEvent = async (ev: Record<string, unknown>) => {
        if (ev.type === "step") setStep(String(ev.text ?? ""));
          ...
          const verb = action === "create" ? "إنشاء ملف" : "تعديل ملف";
      ...
      await new Promise<void>((resolve) => {
        const unsub = subscribeJob(jobId, {
          ...
          onStale: async (row) => {
            const msg = "عامل البرمجة توقف قبل إنهاء المهمة. تم حفظ المحادثة والملفات، ويمكنك الاستكمال من نفس المشروع.";
            try { localStorage.removeItem(`buildAgent:activeJob:${projectId}`); } catch { /* ignore */ }
            await failStaleJob(row.id, msg);
            toast.error(msg);
            setMessages((m) => m.filter((x) => !x.pending));
  ...
  const isBaseProjectRoute = !isChatRoute && !isPreviewRoute && !location.pathname.endsWith("/timeline") && !location.pathname.endsWith("/domains");
  ...
  return (
    ...
    <AppSidebar
      ...
      currentMode="megsy-pr"
    />
    <div className="mb-paper flex h-[100dvh] w-full flex-col overflow-hidden" data-build-redesign="desktop-v4">
      ...
      <header className="mb-rail shrink-0 px-3 flex items-center gap-1.5">
        ...
        <ProjectMenu
          ...
          onRename={async () => {
            const next = window.prompt("اسم المشروع الجديد", project?.name ?? "");
      ...
      {open && (
        ...
        >
          <div className="px-3 py-2.5 border-b border-white/10 mb-1">
            <div className="text-[10px] uppercase tracking-wider text-white/40 mb-1">المشروع</div>
            ...
            <div className="mt-2 flex items-center gap-1.5 text-[11.5px] text-white/70">
              ...
              <span className="text-white/40">كريدت</span>
            </div>
          </div>
          {[
            { label: "تغيير اسم المشروع", icon: Pencil, onClick: () => { onRename(); setOpen(false); } },
            { label: "إعدادات المشروع", icon: SettingsIcon, onClick: () => { onSettings(); setOpen(false); } },
            { label: "خروج من المشروع", icon: LogOut, onClick: () => { onExit(); setOpen(false); }, danger: true },
