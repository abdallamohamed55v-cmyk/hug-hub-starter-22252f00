...
interface Message {
  ...
  docsClarify?: { reason: string; questions: import("@/lib/agent/docs/types").DocsClarifyQuestion[]; originalPrompt: string };
  docsJobId?: string;
  chatJobId?: string;
...
const ChatPage = () => {
  ...
  useEffect(() => () => stopDocsStatusFallback(), [stopDocsStatusFallback]);
    ...
    if (chatMode === "slides") {
      ...
      try {
        ...
        if (placeholderId) {
          ...
          setMessages((prev) => prev.map((m) => m.id === placeholderId ? { ...m, slidesJobId: jobId, slidesPendingTopic: userInput } : m));
        }
            ...
            onClarify: (c) => {
              ...
              setMessages((prev) => prev.map((m) =>
                m.clientId === `assistant-${localTurnId}` ? {
                  ...
                  docsClarify: { reason: c.reason, questions: c.questions, originalPrompt: userInput },
                } : m
              ));
            },
    ...
    await streamChat({
      ...
      onJobStart: isDeepResearch ? (jobId) => {
        ...
        void (async () => {
          ...
          } catch (e) { console.warn("[research] placeholder save failed", e); }
        })();
      } : undefined,
      chatMode: chatMode,
      ...
      onDone: async () => {
        ...
        if (isDeepResearch) {
          const placeholder = messages.find((m) => m.clientId === `assistant-${localTurnId}`) as any;
          const cur = placeholder?.chatJobId;
          if (cur) removeActiveChatJob(cur);
          // Drop the DB placeholder row — the final assistant message will be inserted below.
          if (placeholder?.id) {
            void supabase.from("messages").delete().eq("id", placeholder.id).then(() => {});
          }
        }
                            ...
                            onSubmit={async (answers) => {
                              ...
                              try {
                                ...
                                await streamDoc({
                                  ...
                                  onHtmlDone: (full) => { finalHtml = full; flush(full, true); },
                                  onClarify: (c) => {
                                    setMessages((prev) => prev.map((mm) => matchesTarget(mm) ? {
                                      ...mm, docsArtifact: undefined,
                                      docsClarify: { reason: c.reason, questions: c.questions, originalPrompt: msg.docsClarify!.originalPrompt },
                                    } : mm));
                                  },
