            ...
            {assets.map((a) => (
              ...
              >
                ...
                <div className="absolute inset-x-0 bottom-0 p-2 bg-gradient-to-t from-black/75 to-transparent opacity-0 group-hover:opacity-100 transition-opacity">
                  <p className="text-[10.5px] text-white/95 line-clamp-2">{a.prompt}</p>
                </div>
                {a.kind === "video" && (
                  <span className="absolute top-2 left-2 px-1.5 py-0.5 rounded-md bg-black/60 text-white text-[10px] font-semibold">
                    VIDEO
                  </span>
                )}
              </motion.button>
      ...
      <AnimatePresence>
        {selected && (
          <motion.div
            ...
            className="fixed inset-0 z-50 bg-black/85 backdrop-blur-md p-4 sm:p-8 grid place-items-center"
          >
            <motion.div
            ...
            >
              ...
              <div className="flex flex-col sm:flex-row sm:items-end gap-3 sm:gap-4">
                <div className="flex-1 min-w-0">
                  ...
                  {selected.prompt && (
                    <p className="text-sm text-white/90 mt-1.5 line-clamp-3">
                      {selected.prompt}
                    </p>
                  )}
                </div>
                <div className="flex gap-2 shrink-0">
                  <button
                    ...
                    className="h-10 px-4 rounded-full bg-white text-black text-sm font-semibold inline-flex items-center gap-2"
                  >
                    <Download className="w-4 h-4" /> Download
                  </button>
                  <button
                    onClick={() => handleDelete(selected)}
                    className="h-10 px-4 rounded-full bg-white/10 text-white text-sm font-medium inline-flex items-center gap-2 hover:bg-white/20"
                  >
