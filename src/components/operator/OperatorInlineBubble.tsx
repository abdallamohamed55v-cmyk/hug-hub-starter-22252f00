    ...
    <>
      <Message className="mb-6 relative">
        ...
        <div className="flex items-center gap-2 text-[12px] text-muted-foreground mb-2">
          ...
          {!isRunning && run.status === "done" && <span className="opacity-70 text-emerald-600">تم</span>}
          {run.status === "failed" && <span className="opacity-70 text-red-500">فشل</span>}
          {isRunning ? (
            <button
        ...
        {(hasPreview || screenshots.length > 0 || files.length > 0) && (
          <div className="mt-3 flex flex-wrap gap-1.5">
            ...
            {screenshots.length > 0 && (
              ...
              >
                <ImageIcon className="w-3 h-3" /> {screenshots.length} صور
              </button>
            )}
            {files.length > 0 && (
              <button
              ...
              >
                <FileText className="w-3 h-3" /> {files.length} ملفات
              </button>
            )}
            <button
              onClick={() => setOpen(true)}
