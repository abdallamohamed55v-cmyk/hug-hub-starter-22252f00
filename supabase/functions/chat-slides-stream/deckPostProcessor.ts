...
function pickLayout(s: Slide, shape: ReturnType<typeof detectContentShape>, recent: string[]): string {
  const candidates = SHAPE_TO_LAYOUTS[shape.contentType] ?? SHAPE_TO_LAYOUTS.text;
  const recentSet = new Set(recent.slice(-2));
  // First: respect AI's choice if it's a valid candidate AND not a recent repeat.
  if (s.layout && candidates.includes(s.layout) && !recentSet.has(s.layout)) {
    return s.layout;
  }
  // Otherwise pick the first candidate not recently used.
...
function applyBudgets(s: Slide): void {
  if (typeof s.title === "string" && wc(s.title) > 9) s.title = trimWords(s.title, 8);
  if (typeof s.subtitle === "string" && wc(s.subtitle) > 22) s.subtitle = trimWords(s.subtitle, 20);
...
export function postProcessDeck(slides: Slide[]): Slide[] {
  ...
  for (const s of slides) {
    ...
    const chosen = pickLayout(s, shape, recentLayouts);
    s.layout = chosen;
  ...
  for (const s of final) {
    ...
    if (layout && recent2.slice(-2).includes(layout)) {
      // pick alternate of same shape
      const shape = detectContentShape(s);
      const alts = SHAPE_TO_LAYOUTS[shape.contentType] ?? SHAPE_TO_LAYOUTS.text;
